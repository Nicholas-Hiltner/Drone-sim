...
void Drone::getNextDelivery() {
  std::cout << "model: " << (model != nullptr) << std::endl;
...
void Drone::update(double dt) {
  std::cout << position << " " << (toPackage == nullptr) << " " << available
            << std::endl;
  if (available) {
    std::cout << "we here" << std::endl;
    getNextDelivery();
  }
  if (goCharge && toCharger) {
    toCharger->move(this, dt);
...
void Drone::setToCharger(IStrategy* strat) { toCharger = strat; }

void Drone::setCharging(bool need) { charging = need; }

void Drone::setGoCharge(bool go) { goCharge = go; }
